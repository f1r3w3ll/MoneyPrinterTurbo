...
def _load(script, draft_id=None):
    ...
    st.session_state.output_thumbnail_text = script.get('title', '')
...
def _script_sources(backend, settings):
    st.subheader('1 · Prepare o roteiro')
    ...
    with st.expander('Gerar roteiro com IA'):
        with st.form('generate_script'):
            topic = st.text_input('Tema do vídeo')
            ...
            if st.form_submit_button('Gerar roteiro'):
                if not topic.strip():
                    ...
                    with st.spinner('Escrevendo roteiro…'):
                        try:
                            result = ScriptGeneratorService().generate_script(ScriptGenerationRequest(topic=topic, duration_minutes=minutes, llm_provider=provider, custom_instructions=instructions, language=language, style=style, target_audience=audience))
                            script = result[0] if isinstance(result, tuple) else result
                            _load(script.model_dump())
...
def _editor(backend):
    ...
    st.subheader('2 · Revise as cenas')
...
def _produce(backend, settings, script):
    st.subheader('3 · Produza o vídeo')
...
def render():
    ...
    <section class="studio-hero">
      ...
      <div class="studio-flow" aria-label="Etapas da produção">
        <span>1 · Roteiro</span><span>2 · Revisão</span><span>3 · Produção</span><span>4 · Download</span>
      </div>
    </section>
    ...
    try:
        ...
        with create_tab:
            _script_sources(backend, settings)
